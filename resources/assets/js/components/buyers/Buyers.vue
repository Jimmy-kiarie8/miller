   ...
   <v-container fluid fill-height>
    ...
    <v-layout justify-center align-center v-show="!loader">
     <div class="container">
       ...
       <div>
        <v-card-title>
         <v-btn color="primary" flat @click="openUser">Add Buyer</v-btn>
         <v-spacer></v-spacer>
         <v-text-field
         v-model="search"
         append-icon="search"
...
<AddBuyers @closeRequest="close" :openAddRequest="dispAdd" @alertRequest="showAlert"></AddBuyers>
<EditBuyers @closeRequest="close" :openEditRequest="pdialog2" @alertRequest="showAlert" :editedItemCon="editedItem"></EditBuyers>
<!-- <ShowUserProfile @closeRequest="close" :openProRequest="docsdialog2" :users="AllBuyers" :docsPass="seeDocuments"></ShowUserProfile> -->
</div>
</template>

<script>
let AddBuyers = require('./AddBuyers.vue')
// let ShowUserProfile = require('./ShowUserProfile.vue')
let EditBuyers = require('./EditBuyers.vue')
export default {
  props: ['user', 'role'],
  components: {
    AddBuyers, EditBuyers
    // , ShowUserProfile
  }, 
  data() {
    return{
...
methods: {  
  ...
  editItem(item) {
    ...
    this.editedIndex = this.AllBuyers.indexOf(item)
