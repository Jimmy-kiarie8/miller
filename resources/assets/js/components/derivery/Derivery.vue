...
<EditDerivery 
  ...
  :units="AllUnits"
></EditDerivery>
<!-- <ShowUserProfile @closeRequest="close" :openProRequest="docsdialog2" :users="Allderivery" :docsPass="seeDocuments"></ShowUserProfile> -->
</div>
</template>

<script>
let AddDerivery = require('./AddDerivery.vue')
// let ShowUserProfile = require('./ShowUserProfile.vue')
let EditDerivery = require('./EditDerivery.vue')
export default {
  props: ['user', 'role'],
  components: {
    AddDerivery, EditDerivery
    // , ShowUserProfile
  }, 
  data() {
    return{
...
methods: {  
  ...
  editItem(item) {
    ...
    this.editedIndex = this.Allderivery.indexOf(item)
  ...
  del(key, id) {
    if (confirm('Are you sure you want to delete this item?')) {
      ...
      .then((response) => {
        this.Allderivery.splice(index, 1)
        this.loader = false
...
mounted() {
  ...
  .then((response) => {
    this.Allderivery = response.data
  })
  .catch((error) => {
    this.errors = error.response.data.errors
  })
